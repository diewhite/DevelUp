...
//				String storeFilename = createStoreFileNmae(originalFilename);
				...
				multipartFile.transferTo(new File(path+File.separator+originalFilename));  //중복되지않도록 uuid로 만든 storeFilename으로 생성
				
				filedtolist.add(new DealFile_DTO(originalFilename)); //dto생성자 2개짜리 만들어야함  // uuid사용못해 1개짜리 생성자 추가
